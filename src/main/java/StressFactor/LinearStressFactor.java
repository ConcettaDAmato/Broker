...
public class LinearStressFactor extends StressFactor{

	/** General constructor used to pass the value of variables */
	public LinearStressFactor (double[] thetaWp, double[] thetaFc, double[] z, double[] deltaZ, int NUM_CONTROL_VOLUMES, double totalDepth) {
		super(thetaWp, thetaFc, z, deltaZ, NUM_CONTROL_VOLUMES, totalDepth);}	
	...
	for (int i = 0; i <= NUM_CONTROL_VOLUMES-2; i++) {
		...
		if(z[i] >= zR || z[i] >= zE) {
			stressFactor[i]=(theta[i]-thetaWp[i])/(thetaFc[i]-thetaWp[i]);
